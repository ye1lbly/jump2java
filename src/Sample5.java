...
public class Sample5 {
    public static void main(String[] args){
        ...
        double d = Double.parseDouble(number); //실수 형태의 문자열 실수로 변환 + Float.parseFloat를 사용해도 됨
        int p = (int) d; //실수를 정수로 바뀔 땐 반드시 int형으로 캐스팅해줘야함. 이때 (int)이 캐스팅
        System.out.println(p);

        final List<String> a = List.of("a", "b");
        //final은 자료형에 값을 단 한 번만 설정할 수 있게 강제하는 키워드로 값을 설정하면 재설정이 불가능함
